...
class UploadSimulatorTest extends PHPUnit\Framework\TestCase
{
    ...
    public function test_upload_simplified()
    {
        ...
        $this->assertEquals(1, $calls_count, 'callback function calls more than expected');
    }

    public function test_upload_simplified_two_consecutive_uploads()
    {
        $this->assertEquals(42, 42);
        $test_resource = self::$TEST_RESOURCE_ID_PATH;
        $test_resource_id = self::$TEST_RESOURCE_ID;
        ...
        $this->assertEquals(2, $calls_count, 'callback function calls more than expected');
    }
}